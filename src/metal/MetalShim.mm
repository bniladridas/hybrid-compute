...
struct MetalStream {
    ...
    MetalStream(bool nonBlocking = false) : isNonBlocking(nonBlocking), lastCommandBuffer(nil) {}
...
struct MetalEvent {
    ...
    MetalEvent() : event(nil), signalValue(1), isRecording(false), isCompleted(false) {}
...
namespace {
    ...
    private:
        ...
        MetalShim() : m_serialQueue(dispatch_queue_create("com.hybridcompute.metalshim", DISPATCH_QUEUE_SERIAL)) {
            @autoreleasepool {
    ...
    public:
        ...
        cudaError_t malloc(void** devPtr, size_t size) {
            ...
            @autoreleasepool {
                if (!m_device) return cudaErrorInitializationError;

                // Create a new Metal buffer with shared storage mode
                id<MTLBuffer> buffer = [m_device newBufferWithLength:size
                                                          options:MTLResourceStorageModeShared];
                if (!buffer) return cudaErrorMemoryAllocation;
        ...
        cudaError_t memcpy(void* dst, const void* src, size_t count, cudaMemcpyKind kind) {
            ...
            @autoreleasepool {
                if (!m_commandQueue) return cudaErrorInitializationError;
                ...
                switch (kind) {
                    case cudaMemcpyHostToDevice:
                        if (!dstBuffer) return cudaErrorInvalidDevicePointer;
                        ::memcpy([dstBuffer contents], src, count);
                        break;

                    case cudaMemcpyDeviceToHost:
                        if (!srcBuffer) return cudaErrorInvalidDevicePointer;
                        ::memcpy(dst, [srcBuffer contents], count);
                        break;

                    case cudaMemcpyDeviceToDevice:
                        if (!srcBuffer || !dstBuffer) return cudaErrorInvalidDevicePointer;
        ...
        cudaError_t memcpyAsync(void* dst, const void* src, size_t count, cudaMemcpyKind kind, MetalStream* stream) {
            if (count == 0) return cudaSuccess;
            ...
            @autoreleasepool {
                ...
                if (!commandBuffer) return cudaErrorInitializationError;
                ...
                if (!blitEncoder) return cudaErrorInitializationError;
        ...
        cudaError_t memset(void* devPtr, int value, size_t count) {
            ...
            @autoreleasepool {
                ...
                if (!buffer) return cudaErrorInvalidDevicePointer;
        ...
        cudaError_t memsetAsync(void* devPtr, int value, size_t count, MetalStream* stream) {
            if (!devPtr || count == 0) return cudaSuccess;
        ...
        cudaError_t launchKernel(
                ...
                unsigned int sharedMem, MetalStream* stream, void** args, void** extra) {
            // Not implemented yet
            return cudaErrorNotSupported;
        }
        ...
        cudaError_t eventRecord(MetalEvent* event, MetalStream* stream) {
            ...
            @autoreleasepool {
                ...
                if (!event->event) return cudaErrorInitializationError;
...
cudaError_t cudaShimInit() {
    return MetalShim::instance().init();
}

void cudaShimShutdown() {
    MetalShim::instance().shutdown();
}
...
cudaError_t cudaShimMemcpy(void* dst, const void* src, size_t count, cudaMemcpyKind kind) {
    return MetalShim::instance().memcpy(dst, src, count, kind);
}

cudaError_t cudaShimMemcpyAsync(void* dst, const void* src, size_t count, cudaMemcpyKind kind, void* stream) {
    return MetalShim::instance().memcpyAsync(dst, src, count, kind, static_cast<MetalStream*>(stream));
}
...
cudaError_t cudaShimMemsetAsync(void* devPtr, int value, size_t count, void* stream) {
    return MetalShim::instance().memsetAsync(devPtr, value, count, static_cast<MetalStream*>(stream));
}

cudaError_t cudaShimLaunchKernel(
        const void* func,
        unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ,
        unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ,
        unsigned int sharedMem, void* stream, void** args, void** extra) {
...
cudaError_t cudaShimStreamSynchronize(void* stream) {
    return MetalShim::instance().streamSynchronize(static_cast<MetalStream*>(stream));
}

cudaError_t cudaShimEventRecord(void* event, void* stream) {
    return MetalShim::instance().eventRecord(static_cast<MetalEvent*>(event), static_cast<MetalStream*>(stream));
}

cudaError_t cudaShimEventSynchronize(void* event) {
    return MetalShim::instance().eventSynchronize(static_cast<MetalEvent*>(event));
}

cudaError_t cudaShimEventElapsedTime(float* ms, void* start, void* end) {
    return MetalShim::instance().eventElapsedTime(ms, static_cast<MetalEvent*>(start), static_cast<MetalEvent*>(end));
}

// Function pointer initialization
cudaError_t (*cudaMallocPtr)(void** devPtr, size_t size) = nullptr;
cudaError_t (*cudaFreePtr)(void* devPtr) = nullptr;
cudaError_t (*cudaMemcpyPtr)(void* dst, const void* src, size_t count, cudaMemcpyKind kind) = nullptr;
cudaError_t (*cudaMemcpyAsyncPtr)(void* dst, const void* src, size_t count, cudaMemcpyKind kind, cudaStream_t stream) = nullptr;
cudaError_t (*cudaMemsetPtr)(void* devPtr, int value, size_t count) = nullptr;
cudaError_t (*cudaMemsetAsyncPtr)(void* devPtr, int value, size_t count, cudaStream_t stream) = nullptr;
cudaError_t (*cudaLaunchKernelPtr)(
    const void* func,
    unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ,
    unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ,
    unsigned int sharedMem, cudaStream_t stream, void** args, void** extra) = nullptr;
cudaError_t (*cudaDeviceSynchronizePtr)() = nullptr;
cudaError_t (*cudaStreamSynchronizePtr)(cudaStream_t stream) = nullptr;
cudaError_t (*cudaEventRecordPtr)(cudaEvent_t event, cudaStream_t stream) = nullptr;
cudaError_t (*cudaEventSynchronizePtr)(cudaEvent_t event) = nullptr;
cudaError_t (*cudaEventElapsedTimePtr)(float* ms, cudaEvent_t start, cudaEvent_t end) = nullptr;
...
extern "C" __attribute__((constructor)) void initCudaShim() {
    ...
    // Memory operations
    cudaMemcpyPtr = [](void* dst, const void* src, size_t count, cudaMemcpyKind kind) -> cudaError_t {
        return MetalShim::instance().memcpy(dst, src, count, kind);
    };

    cudaMemcpyAsyncPtr = [](void* dst, const void* src, size_t count, cudaMemcpyKind kind, cudaStream_t stream) -> cudaError_t {
        return MetalShim::instance().memcpyAsync(dst, src, count, kind, static_cast<MetalStream*>(stream));
    };
    ...
    cudaMemsetAsyncPtr = [](void* devPtr, int value, size_t count, cudaStream_t stream) -> cudaError_t {
        return MetalShim::instance().memsetAsync(devPtr, value, count, static_cast<MetalStream*>(stream));
    };
    ...
    cudaLaunchKernelPtr = [](
        ...
        unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ,
        unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ,
        unsigned int sharedMem, cudaStream_t stream, void** args, void** extra) -> cudaError_t {
        return MetalShim::instance().launchKernel(
            func, gridDimX, gridDimY, gridDimZ,
            blockDimX, blockDimY, blockDimZ,
            sharedMem, static_cast<MetalStream*>(stream), args, extra);
    ...
    // Event management
    cudaEventRecordPtr = [](cudaEvent_t event, cudaStream_t stream) -> cudaError_t {
        return MetalShim::instance().eventRecord(static_cast<MetalEvent*>(event), static_cast<MetalStream*>(stream));
    };
    ...
    cudaEventElapsedTimePtr = [](float* ms, cudaEvent_t start, cudaEvent_t end) -> cudaError_t {
        return MetalShim::instance().eventElapsedTime(ms, static_cast<MetalEvent*>(start), static_cast<MetalEvent*>(end));
    };
}
