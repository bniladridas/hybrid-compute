...
void verifyResults(const float* A, const float* B, const float* C, int numElements) {
...
int main() {
    ...
    cudaMemcpyAsync(d_A, h_A.data(), halfSizeBytes, cudaMemcpyHostToDevice, stream1);
    cudaMemcpyAsync(d_B, h_B.data(), halfSizeBytes, cudaMemcpyHostToDevice, stream1);
    vectorAdd<<<(halfSize + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock, 0, stream1>>>(
        d_A, d_B, d_C, halfSize);
    cudaMemcpyAsync(h_C.data(), d_C, halfSizeBytes, cudaMemcpyDeviceToHost, stream1);

    // Second half on stream2
    cudaMemcpyAsync(d_A + halfSize, h_A.data() + halfSize, halfSizeBytes, cudaMemcpyHostToDevice, stream2);
    cudaMemcpyAsync(d_B + halfSize, h_B.data() + halfSize, halfSizeBytes, cudaMemcpyHostToDevice, stream2);
    vectorAdd<<<(halfSize + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock, 0, stream2>>>(
        d_A + halfSize, d_B + halfSize, d_C + halfSize, halfSize);
    cudaMemcpyAsync(h_C.data() + halfSize, d_C + halfSize, halfSizeBytes, cudaMemcpyDeviceToHost, stream2);
