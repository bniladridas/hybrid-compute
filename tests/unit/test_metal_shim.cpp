...
TEST_F(MetalShimTest, TestMemcpy) {
    ...
    // Copy host to device
    EXPECT_EQ(cudaMemcpy(d_ptr, h_src, size * sizeof(float), cudaMemcpyHostToDevice), cudaSuccess);

    // Copy device to host
    EXPECT_EQ(cudaMemcpy(h_dst, d_ptr, size * sizeof(float), cudaMemcpyDeviceToHost), cudaSuccess);
