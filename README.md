...
- **Notes**: Google Benchmark is enabled by default on all platforms with static MSVC runtime on Windows to avoid conflicts. Modify ENABLE_BENCHMARK option in benchmark.cmake to adjust. CI runs with parallel testing, non-interactive prompts, and containerized builds.
