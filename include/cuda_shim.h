...
typedef cudaError_t (*cudaLaunchKernel_t)(const void* func,
    unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ,
    unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ,
    unsigned int sharedMem, cudaStream_t stream, void** args, void** extra);
...
typedef cudaError_t (*cudaStreamAddCallback_t)(cudaStream_t stream, cudaStreamCallback_t callback, void* userData, unsigned int flags);
...
#define cudaMemcpyAsync(dst, src, count, kind, stream) cudaMemcpyAsyncPtr(dst, src, count, kind, stream)
#define cudaMemset(devPtr, value, count) cudaMemsetPtr(devPtr, value, count)
#define cudaMemsetAsync(devPtr, value, count, stream) cudaMemsetAsyncPtr(devPtr, value, count, stream)
...
#define cudaStreamCreateWithFlags(pStream, flags) cudaStreamCreateWithFlagsPtr(pStream, flags)
...
#define cudaStreamAddCallback(stream, callback, userData, flags) cudaStreamAddCallbackPtr(stream, callback, userData, flags)

// Event management macros
#define cudaEventCreate(event) cudaEventCreatePtr(event)
#define cudaEventCreateWithFlags(event, flags) cudaEventCreateWithFlagsPtr(event, flags)
...
#define cudaEventElapsedTime(ms, start, end) cudaEventElapsedTimePtr(ms, start, end)
