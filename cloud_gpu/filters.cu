 ...
 *
 * This file provides GPU-accelerated image filters: Gaussian blur and edge detection (Sobel).
...
__global__ void gaussianBlurKernel(uchar* input, uchar* output, int width, int height) {
...
__global__ void sobelEdgeKernel(uchar* input, uchar* output, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    output[(y * width + x) * 3] = output[(y * width + x) * 3 + 1] = output[(y * width + x) * 3 + 2] = (uchar)min(magnitude, 255.0f);
}
