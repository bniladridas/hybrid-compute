...
__global__ void rotateKernel(uchar* input, uchar* output, int in_w, int in_h, int out_w, int out_h, float angle) {
...
int applyRotation(const cv::Mat& input, cv::Mat& output, float angle) {
    ...
    rotateKernel<<<grid, block>>>(d_input, d_output, in_w, in_h, out_w, out_h, angle);
